...
protected:
	...
	std::vector < std::shared_ptr<Expression>> complex_cond; //concatenated conditions from outer scopes - test_generator
	std::vector<std::shared_ptr<Symbol>> symbol_table; //local symbol table
	//std::unordered_map<std::shared_ptr<Expression>, bool> condition_values; //generated by test_generator
	int first_line;
	int last_line;
...
public:
	...
	int get_first_line() { return first_line; }
	int get_last_line() { return last_line; }
	void set_first_line(int f_line) { first_line = f_line; }
	void set_last_line(int f_line) { last_line = f_line; }

	void add_to_instructions(std::shared_ptr<Instruction> instruction) 
	{ 
		this->instructions.push_back(instruction); 
		this->children.push_back(instruction);
	}
	void add_to_blocks(std::shared_ptr<Block> block) 
	{ 
	...
	void set_condition(std::shared_ptr<Expression> cond) { this->condition = cond; }


	void add_to_complex_conditions(std::shared_ptr<Expression> cond) { this->complex_cond.push_back(cond); }
	std::vector < std::shared_ptr<Expression>> get_complex_conditions() { return complex_cond; }
	
	//void add_to_condition_values(std::pair<std::shared_ptr<Expression>, bool> val) { condition_values.insert(val); }
	//std::unordered_map<std::shared_ptr<Expression>, bool> get_condition_values() { return condition_values; }

	void add_to_parents(std::shared_ptr<Block> scope) { parents.push_back(scope); }
	std::vector<std::shared_ptr<Block>> get_parents() { return parents; }

	void add_to_children(std::shared_ptr<Node> node) { children.push_back(node); }
	...
	std::shared_ptr<Symbol> find_in_symbol_table(std::string name)
	{
	...
	{	
		std::shared_ptr<Symbol> new_symbol = find_in_symbol_table(symbol->get_name());
